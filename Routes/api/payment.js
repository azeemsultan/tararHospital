...
router.post('/pay', async (req, res) => {
    ...
    if(req.body.id){
      const c= await Consult.findOne({doctor:req.body.id,customer:jwt.id,status:"accepted but not paid"});
   const wait=await Consult.updateOne(
      ...
      if(wait){
        ...
        let text="Payement has been done.";
        email(c.customeremail, " Payement ", text);
        email(c.doctoremail, " Payement ", text);
